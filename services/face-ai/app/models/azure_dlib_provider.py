...
#: blur flag fired on 98-100% of faces that had only been darkened, and its
#: exposure flags on 20-75% of them. Between them they left at most 2% of
#: dark or backlit students recognised automatically, and 21% across all
#: conditions. Replaced by measurements of the face itself, the rate is 43%
#: — and in neither case, nor with no flags at all, was a single student
#: marked present as somebody else. Size, pose, occlusion and Azure's
#: recognition-quality rating are applied exactly as before.
...
#: mild. Set at the 5th percentile of the measure on severely blurred faces
#: (2 recogniser pixels or more — a template cost over 0.011 — or 8px of
#: shake) on half the calibration identities. On the other half it flagged
#: 97.6% of severely blurred faces, 41% of moderately blurred ones and 0.4% of
#: unblurred ones from 60px up (11.8% at 40px). Mild and moderate blur are
#: left to the score, which falls with blur by itself.
#:
...
#: Only a face darker and flatter than that (0.8% of the faces in that
#: condition), or more than 60% blown out (the brightest 6% of the brightest
#: validated condition), is flagged.
TOO_DARK_MEAN = 25.0
